...
def step_text(step):
    ...
    if a=='battery_usage': return f"Add {step.get('days',1)} day(s) of battery usage for {step.get('target')} at intensity {step.get('intensity',1)}" + (f" with {step.get('battery_mv')} mV" if step.get('battery_mv') is not None else '') + (f" (expect rejection: {step.get('expect_error')})" if step.get('expect_error') else '')
    if a=='battery_counter_reset': return f"Simulate battery telemetry counter reset/reboot for {step.get('target')}"
    if a=='battery_profile_patch': return f"Change device power calibration for {step.get('target')}: {json.dumps(step.get('values',{}),sort_keys=True)}"
