...
function batteryClass(p,drain='NORMAL'){
  const alert=Number(state.schedules?.battery_alert_percent ?? 20);
  if(drain==='HIGH' || Number(p)<=alert)return 'red';
  return Number(p)<=55?'amber':'';
}
...
function renderHome(){
  ...
  const healthHeadline=highDrain.length?`Battery draining faster: ${highDrain.join(', ')}`:(health.low_percent<=batteryAlert?`Low battery: ${health.low_name} ${health.low_percent}%`:`Lowest battery: ${health.low_name} ${health.low_percent}%`);
...
function renderDevices(){
 ...
 const alertPercent=Number(state.schedules?.battery_alert_percent ?? 20);
 ...
 <div class="device-list">${state.devices.map(d=>{const batteryAlert=Number(d.battery)<=alertPercent,drainHigh=d.drain_status==='HIGH',attention=!d.active||batteryAlert||drainHigh;const usage=d.daily_mah==null?'Learning usage pattern':`${Number(d.daily_mah).toFixed(1)} mAh/day`;const confidence=d.confidence||'LOW';return `<div class="device-card ${attention?'alert':''}" data-device-id="${d.id}">
   <div class="round-icon">${d.id==='entry'?'🚪':d.id==='hub'?'📶':'🏃'}</div>
   <div><div class="device-name">${d.name}</div><div class="muted">${d.type}</div><div class="${d.active?'status-good':'status-bad'}">● ${d.active?'Active':'Inactive'}</div>${drainHigh?'<div class="status-bad">Battery draining faster than usual</div>':''}</div>
   <div class="battery-col"><div class="battery"><div class="battery-icon"><div class="battery-fill ${batteryClass(d.battery,d.drain_status)}" style="width:${Math.max(2,Number(d.battery)||0)}%"></div></div><div class="battery-pct">${d.battery}%</div></div>
   <div class="${batteryAlert||drainHigh?'status-bad':'status-good'}">Estimated ${d.id==='hub'?'backup':'time left'} ${d.left}</div><div class="muted">${usage} · ${confidence} confidence</div><div class="muted">${d.battery_mv||'—'} mV · Drain ${String(d.drain_status||'LEARNING').toLowerCase()}</div><div class="muted">Usage pattern: ${d.wakeups_per_day==null?'—':Math.round(d.wakeups_per_day)} wakes/day · ${d.retries_per_day==null?'—':Number(d.retries_per_day).toFixed(1)} RF retries/day</div><div class="muted">◷ ${d.active?'Last updated':'Last seen'} ${d.updated}</div></div><div class="arrow">›</div>
 </div>`}).join('')}</div>`;
}
...
function scheduleEditor(){
 ...
 return `<div id="scheduleEditor" class="schedule-editor">
   ...
   <form id="scheduleForm" onsubmit="saveScheduleSettings(event)">
    ...
    <section><h3>Device power & battery</h3><div class="form-grid"><label>Low-battery alert below (%)<input name="battery_alert_percent" type="number" min="1" max="50" step="1" value="${r.battery_alert_percent}"></label></div><p class="rule-help">Battery-life prediction uses each device's calibrated power profile plus its measured voltage and usage counters. Current calibration is host-simulation data until hardware bench measurements are loaded.</p></section>
...
window.saveScheduleSettings=async(ev)=>{ev.preventDefault();const f=new FormData(ev.target),r={...state.schedules};
 ...
 r.night_visit_merge_seconds=Math.round(Number(f.get('night_visit_merge_seconds'))*60);r.night_bathroom_visit_threshold=Number(f.get('night_bathroom_visit_threshold'));r.night_common_visit_threshold=Number(f.get('night_common_visit_threshold'));r.battery_alert_percent=Number(f.get('battery_alert_percent'));
 try{
...
function render(){
  ...
  if(!scheduleEditorOpen)renderSettings();
