...
from local_lab import Lab, WebLab, QuietRequests, ThreadingWSGIServer
...
class HttpTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.server=make_server("127.0.0.1",0,cls.web,server_class=ThreadingWSGIServer,handler_class=QuietRequests)
    ...
    def test_door_left_open_and_close_duration(self):
        _,base=self.request("/sim/state")
        settings={k:v for k,v in base["settings"].items() if k!="config_version"}
        settings["door_open_timeout_seconds"]=300
