# Ghar Sajag v1.5.4-routine-battery Manual Functional Validation Plan
...
### M086 — Battery analytics predicts remaining days from voltage plus calibrated usage history
Scenario ID: `battery-prediction-baseline`  
Requirements: E01, E10, NFR-05

**Steps**
1. No action; inspect reset state.

**Expected**
- `battery_analytics.kitchen.percent` gt `50`
- `battery_analytics.kitchen.daily_mah` gt `0`
- `battery_analytics.kitchen.estimated_days` gt `30`
- `battery_analytics.kitchen.confidence` eq `HIGH`
- `battery_analytics.kitchen.drain_status` eq `NORMAL`
- `battery_analytics.kitchen.recent_wakeups_per_day` gt `0`

Result: ☐ PASS ☐ FAIL   Notes: ______________________________

### M087 — Sustained high wake/radio usage is detected and shortens battery-life prediction
Scenario ID: `battery-high-drain-detected`  
Requirements: E01, E10, NFR-05

**Steps**
1. Add 2 day(s) of battery usage for kitchen at intensity 20

**Expected**
- `battery_analytics.kitchen.drain_status` eq `HIGH`
- `battery_analytics.kitchen.recent_daily_mah` gt `20`
- `battery_analytics.kitchen.baseline_daily_mah` lt `10`
- `battery_analytics.kitchen.estimated_days` lt `100`
- `battery_analytics.kitchen.recent_retries_per_day` ge `40`

Result: ☐ PASS ☐ FAIL   Notes: ______________________________

### M088 — Low battery voltage respects reserve and does not over-promise runtime
Scenario ID: `battery-low-voltage-reserve`  
Requirements: E01, E10, NFR-05

**Steps**
1. Add 3 day(s) of battery usage for kitchen at intensity 1 with 3500 mV

**Expected**
- `battery_analytics.kitchen.percent` eq `5`
- `battery_analytics.kitchen.remaining_mah` eq `0.0`
- `battery_analytics.kitchen.estimated_days` eq `None`

Result: ☐ PASS ☐ FAIL   Notes: ______________________________

### M089 — Battery analytics tolerates device counter reset without negative energy
Scenario ID: `battery-counter-reset-safe`  
Requirements: E01, E10, NFR-05

**Steps**
1. Simulate battery telemetry counter reset/reboot for kitchen

**Expected**
- `battery_analytics.kitchen.modeled_consumed_mah` ge `0`
- `battery_analytics.kitchen.estimated_days` gt `0`
- `battery_analytics.kitchen.confidence` eq `LOW`

Result: ☐ PASS ☐ FAIL   Notes: ______________________________

### M090 — Device battery capacity calibration changes runtime prediction without code change
Scenario ID: `battery-profile-capacity-configurable`  
Requirements: E01, E10, NFR-05

**Steps**
1. Change device power calibration for kitchen: {"usable_capacity_mah": 4000.0}

**Expected**
- `battery_analytics.kitchen.estimated_days` gt `350`
- `battery_analytics.kitchen.daily_mah` lt `10`

Result: ☐ PASS ☐ FAIL   Notes: ______________________________

### M091 — Household low-battery alert threshold is configuration, not a hardcoded value
Scenario ID: `battery-alert-threshold-configurable`  
Requirements: F04, E01, NFR-05

**Steps**
1. Change household setting(s): {"battery_alert_percent": 35}

**Expected**
- `settings.battery_alert_percent` eq `35`
- `settings.config_version` ge `2`
...
### M092 — Invalid battery-usage telemetry controls are rejected safely
Scenario ID: `battery-invalid-usage-rejected`  
Requirements: E07, E10, NFR-05

**Steps**
1. Add 0 day(s) of battery usage for kitchen at intensity 1 (expect rejection: invalid battery usage)
...
- ☐ Devices page shows battery %, voltage, estimated time left, mAh/day, confidence and usage-pattern counters.
- ☐ A high-drain device is visually flagged, but does not change the household care banner.
- ☐ Low-battery alert threshold can be changed in Device Schedules and persists after Save.

## Dashboard hierarchy / presentation regression checks

- UI-01: Overall banner contains only household status, not routine/device-health details.
- UI-02: Routine cards remain below the banner in the expected order.
- UI-03: Missing morning highlights the Morning Routine card and generic red banner.
- UI-04: Door concern highlights Main Door; UI-05: night concern highlights Night Activity.
- UI-06: Device-health-only issues (low battery/high drain) do not change the care banner.
- UI-07: Running all validation restores the dashboard to Reset/PASS.

## Physical-hardware acceptance (cannot be completed in host simulation)

Software battery analytics is automated, but the current-calibration inputs are still synthetic. Before an unattended pilot, measure real deep-sleep/awake/sensor/RF currents, ADC voltage accuracy, battery usable capacity, brown-out behaviour and charging/UPS behaviour on the exact hardware. Also complete real PIR/reed/button debounce, ESP-NOW RF/RSSI, reset reasons, flash durability, OTA, notification delivery and installed-home coverage tests.
