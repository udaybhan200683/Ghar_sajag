...
class GharSajagService:
    ...
    def __init__(self, store: InMemoryStore | None = None, lock: threading.RLock | None = None) -> None:
        ...
        self.battery = BatteryAnalyticsService(self.store)
    ...
    def accept_hub_event(self, event: CloudEvent) -> tuple[CloudEvent, bool]:
        ...
        if event.kind == "CALL_FAMILY":
            ...
            if not created_before:
                self.notifications.schedule_for_incident(incident.incident_id)
        elif event.kind in {"DOOR_LEFT_OPEN", "UNUSUAL_NIGHT_BATHROOM_ACTIVITY", "UNUSUAL_NIGHT_COMMON_ACTIVITY", "POST_DOOR_INACTIVITY"}:
            incident, created_before = self.incidents.create(
                event.home_id,
                f"routine-concern:{event.kind}:{event.event_id}",
                event.kind,
                event.server_received_at,
            )
            if not created_before:
