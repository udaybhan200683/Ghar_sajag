...
| `make e2e-test PRODUCT=base` | JSON `status: PASS`, 92 scenario entries | Connected C++ → Python adapter flows |
| `make http-e2e-test PRODUCT=base` | `Ran 16 tests`, `OK` | Real local HTTP routes, API actions, assets, invalid input and local-origin boundaries; includes running the 92-scenario release suite |
...
| File | Meaning |
|---|---|
| `logs/e2e_report.json` / `.txt` | Latest 92-scenario result; also downloadable in browser |
...
This runs contract checks, C++ unit tests, backend/database/logging tests, app tests, product profiles, feature variants, five dummy sensor streams, the 92-case connected functional catalog, HTTP integration, sanitizers and a trace build. The detailed design is in `docs/VALIDATION_FRAMEWORK.md`.
