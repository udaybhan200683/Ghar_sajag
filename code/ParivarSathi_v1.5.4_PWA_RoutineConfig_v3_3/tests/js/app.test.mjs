...
import { buildDesiredConfig, configStatus, validateActivityRules, DEFAULT_ACTIVITY_RULES } from "../../app/src/features/routines/index.mjs";
...
test("A03 serializes configurable household routine thresholds", () => {
  const r={...DEFAULT_ACTIVITY_RULES,nightBathroomVisitThreshold:2,nightCommonVisitThreshold:3,doorOpenTimeoutSeconds:7200,postDoorInactivitySeconds:9000};
  assert.equal(validateActivityRules(r).valid,true);
  const c=buildDesiredConfig("h2",4,"Asia/Kolkata","HOME",{startMinute:420,endMinute:600,graceMinutes:30},r);
  assert.equal(c.activity_rules.night_bathroom_visit_threshold,2);
  assert.equal(c.activity_rules.post_door_inactivity_seconds,9000);
});
