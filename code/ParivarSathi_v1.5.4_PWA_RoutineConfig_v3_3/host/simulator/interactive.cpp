...
using namespace gs;
namespace {
const std::array<std::string,6> names{"room1", "kitchen", "entry", "pooja", "bathroom", "common"};
...
const char* signal_name(RuleSignalKind kind) {
    switch (kind) {
        ...
        case RuleSignalKind::MorningRoutineCompleted: return "MORNING_ROUTINE_COMPLETED";
        case RuleSignalKind::UnusualNightBathroomActivity: return "UNUSUAL_NIGHT_BATHROOM_ACTIVITY";
        case RuleSignalKind::UnusualNightCommonActivity: return "UNUSUAL_NIGHT_COMMON_ACTIVITY";
        case RuleSignalKind::PostDoorInactivity: return "POST_DOOR_INACTIVITY";
    }
...
struct Simulation {
    ...
    void print() {
        ...
        std::cout<<"{\"now\":"<<now<<",\"wan\":"<<(cloud.connected()?"true":"false")
          ...
          <<",\"post_door_inactivity_alerted\":"<<(hub.activity_state().post_door_inactivity_alerted?"true":"false")
          <<",\"nodes\":[";
        for(std::size_t i=0;i<6;++i) {
            if(i) std::cout<<',';
    ...
    while(std::getline(std::cin,line)) {
        try {
            ...
            if(cmd=="reset") sim=std::make_unique<Simulation>();
                ...
                int me=-1, ms=-1, mend=-1, mwin=-1, ne=-1, ns=-1, nend=-1, nb=-1, nc=-1, merge=-1, pe=-1, post=-1;
                std::string mbed, mbath, mkit, nbathloc, ncommonloc;
                in>>qe>>qs>>qend>>door>>ie>>ds>>de>>inactivity
                  >>me>>ms>>mend>>mwin>>mbed>>mbath>>mkit
                  >>ne>>ns>>nend>>nbathloc>>nb>>ncommonloc>>nc>>merge>>pe>>post;
                if((qe!=0&&qe!=1)||(ie!=0&&ie!=1)||(me!=0&&me!=1)||(ne!=0&&ne!=1)||(pe!=0&&pe!=1)||
                   qs<0||qs>1439||qend<0||qend>1439||ds<0||ds>1439||de<0||de>1439||
                   ms<0||ms>1439||mend<0||mend>1439||ns<0||ns>1439||nend<0||nend>1439||
                   door<30||door>86400||inactivity<300||inactivity>86400||mwin<60||mwin>86400||
                   nb<0||nb>50||nc<0||nc>50||merge<0||merge>7200||post<300||post>86400||
                   mbed.empty()||mbath.empty()||mkit.empty()||nbathloc.empty()||ncommonloc.empty())
                ...
                cfg.morning_sequence_enabled=me!=0; cfg.morning_start_minute=static_cast<std::uint16_t>(ms);
                cfg.morning_end_minute=static_cast<std::uint16_t>(mend); cfg.morning_sequence_window_seconds=mwin;
                cfg.morning_bedroom_location=mbed; cfg.morning_bathroom_location=mbath; cfg.morning_kitchen_location=mkit;
                cfg.night_activity_enabled=ne!=0; cfg.night_start_minute=static_cast<std::uint16_t>(ns);
                cfg.night_end_minute=static_cast<std::uint16_t>(nend); cfg.night_bathroom_location=nbathloc;
                cfg.night_bathroom_visit_threshold=static_cast<std::uint16_t>(nb); cfg.night_common_location=ncommonloc;
                cfg.night_common_visit_threshold=static_cast<std::uint16_t>(nc); cfg.night_visit_merge_seconds=merge;
                cfg.post_door_inactivity_enabled=pe!=0; cfg.post_door_inactivity_seconds=post;
