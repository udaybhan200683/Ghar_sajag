...
enum class RuleSignalKind {
    ...
    UnusualNightBathroomActivity,
    UnusualNightCommonActivity,
    PostDoorInactivity
};
...
struct ActivityRuleState {
    ...
    // Configurable household routine state.
    bool morning_started{false};
    bool morning_bathroom_seen{false};
    bool morning_kitchen_seen{false};
    bool morning_completed{false};
    std::optional<EpochSeconds> morning_started_at;

    std::uint16_t night_bathroom_visits{0};
    std::uint16_t night_common_visits{0};
    std::optional<EpochSeconds> last_night_bathroom_at;
    std::optional<EpochSeconds> last_night_common_at;
    bool night_bathroom_alerted{false};
    bool night_common_alerted{false};
    bool night_window_active{false};

    std::optional<EpochSeconds> door_closed_at;
    bool post_door_activity_seen{false};
    bool post_door_inactivity_alerted{false};
};
