...
def run_case(name: str, flags: str) -> dict:
    # Do not inherit an outer GNU make jobserver into nested verification makes; some CI/container
    # environments otherwise block waiting for unavailable jobserver tokens.
    env = os.environ.copy()
    for key in ("MAKEFLAGS", "MFLAGS", "MAKELEVEL"):
        env.pop(key, None)
    subprocess.run(["make", "clean"], cwd=ROOT, check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, env=env)
    result = subprocess.run(["make", "simulator", f"TRACE_FLAGS={flags}"], cwd=ROOT, check=True, capture_output=True, text=True, env=env)
