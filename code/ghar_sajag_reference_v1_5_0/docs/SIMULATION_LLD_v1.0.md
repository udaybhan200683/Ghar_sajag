...
Applies to code 1.5.0. Companion to documentation edition 2.0, not a replacement for the SRD/LLD volumes.
...
Backend home/caregiver creation uses the shipped API and services. `PrivacyOn/Off` events exercise the existing hub path; Python mirrors the resulting mode through HomeService for the caregiver snapshot. That is a synchronous host propagation adapter, not a completed distributed privacy protocol.
