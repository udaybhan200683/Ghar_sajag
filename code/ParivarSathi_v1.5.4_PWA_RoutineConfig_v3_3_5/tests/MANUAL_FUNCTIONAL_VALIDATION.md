...
These checks validate the PWA presentation layer after the routine-card layout change.
They are additional UI checks; they do not change the canonical functional-scenario count.

### UI-01 — Overall banner contains only overall household status
1. Reset to PASS.
2. Open Home.
3. Confirm the main banner shows `Home looks normal` and `All is well at home.`
4. Confirm the banner does **not** contain Morning Routine, I am OK, Main Door, Night Activity, or Device Health details.

Expected: PASS if the banner is only an overall-status summary.

### UI-02 — Routine cards are ordered below the banner
Confirm the Home cards appear in this order:
1. Morning Routine
2. I am OK
3. Main Door
4. Night Activity
...
Expected: Morning Routine is before I am OK and every routine is below the main banner.

### UI-03 — Missing morning highlights Morning Routine, not banner details
1. Run `morning-missing-covered`.
2. Confirm the overall banner is light red and says `Attention needed at home`.
3. Confirm the banner says `Please check the highlighted routine below.`
4. Confirm the banner does not contain the morning-specific problem text.
5. Confirm the Morning Routine card is light red and shows `Activity not completed`.

### UI-04 — Door concern highlights Main Door
Run a configured door-left-open or post-door-inactivity case.
Expected:
- overall banner = light red/generic attention message;
- Main Door card = light red;
- specific door problem appears in Main Door card.

### UI-05 — Night concern highlights Night Activity
Run a bathroom/common-room unusual-night case.
Expected:
- overall banner = light red/generic attention message;
- Night Activity card = light red;
- specific night problem appears in Night Activity card.

### UI-06 — Device health does not change care banner
Simulate low battery only.
Expected:
- Device Health card = warning/red;
- main banner remains `Home looks normal`.

### UI-07 — Validation cleanup restores normal dashboard
Run all canonical scenarios from the PWA.
After PASS completion:
- dashboard returns to Reset/PASS state;
- no synthetic red routine state remains;
- main banner is `Home looks normal`.
