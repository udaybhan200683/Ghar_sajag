...
// Per-household behaviour policy. These values are versioned configuration, not transport constants.
// Defaults are safe starting values for a newly provisioned home and may be changed by an authorised
// family/caregiver through Settings.
struct ActivityRuleConfig {
    // All household/person-specific values are desired configuration. Defaults only seed a new home;
    // deterministic rule decisions always consume the currently applied configuration.
    bool quiet_hours_enabled{true};
    std::uint16_t quiet_start_minute{60};
    std::uint16_t quiet_end_minute{300};
    EpochSeconds door_open_timeout_seconds{2 * 60 * 60};

    bool daytime_inactivity_enabled{true};
    std::uint16_t daytime_start_minute{8 * 60};
    std::uint16_t daytime_end_minute{21 * 60};
    EpochSeconds daytime_inactivity_seconds{3 * 60 * 60};

    bool morning_sequence_enabled{true};
    std::uint16_t morning_start_minute{6 * 60};
    std::uint16_t morning_end_minute{11 * 60};
    EpochSeconds morning_sequence_window_seconds{4 * 60 * 60};
    std::string morning_bedroom_location{"room1"};
    std::string morning_bathroom_location{"bathroom"};
    std::string morning_kitchen_location{"kitchen"};

    bool night_activity_enabled{true};
    std::uint16_t night_start_minute{22 * 60};
    std::uint16_t night_end_minute{6 * 60};
    std::string night_bathroom_location{"bathroom"};
    std::uint16_t night_bathroom_visit_threshold{4};
    std::string night_common_location{"common"};
    std::uint16_t night_common_visit_threshold{4};
    EpochSeconds night_visit_merge_seconds{5 * 60};

    bool post_door_inactivity_enabled{true};
    EpochSeconds post_door_inactivity_seconds{2 * 60 * 60};
};
...
struct HomeConfig {
    ...
    ActivityRuleConfig activity_rules;
};
