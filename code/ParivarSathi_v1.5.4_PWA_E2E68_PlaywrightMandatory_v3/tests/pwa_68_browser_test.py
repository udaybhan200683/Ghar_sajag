...
class Pwa68BrowserTest(unittest.TestCase):
 ...
 def test_all_68_in_real_pwa_dom(self):
  ...
  try:
   ...
   try:
    ...
    deadline=time.time()+120;status=None;count=0
    while time.time()<deadline:
     ...
     if v:
      ...
      if status in ('PASS','FAIL') and count==68:break
     time.sleep(.2)
    self.assertEqual((status,count),('PASS',68))
    r=cdp(ws,'Runtime.evaluate',{'expression':"document.documentElement.outerHTML",'returnByValue':True});dom=r['result']['value']
    rows=re.findall(r'data-validation-case="([^"]+)" data-case-status="([^"]+)"',dom);self.assertEqual(len(rows),68);self.assertEqual([c for c,s in rows if s!='PASS'],[])
    ...
    print('PASS PWA browser E2E: 68/68 rendered PASS')
   finally: ws.close()
  finally:
   proc.terminate()
