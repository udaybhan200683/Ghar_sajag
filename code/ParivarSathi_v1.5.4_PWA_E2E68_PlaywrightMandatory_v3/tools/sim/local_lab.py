...
class Lab:
    ...
    def reset(self):
        ...
        self.pwa_audit = []
        self.pwa_battery = {
            "hub": {"percent": 82, "runtime": "~18 hrs"},
            "room1": {"percent": 72, "runtime": "~14 days"},
            "kitchen": {"percent": 68, "runtime": "~9 days"},
            "entry": {"percent": 55, "runtime": "~12 days"},
            "pooja": {"percent": 81, "runtime": "~20 days"},
        }
    ...
    def _config_body(self):
        return {
            ...
            "mode": "HOME",
            "morning": {
                "enabled": True, "start_minute": 420, "end_minute": 600, "grace_minutes": 30,
                "qualifying_locations": ["kitchen", "pooja"],
            },
    ...
    def _update_household_settings(self, values):
        ...
        for key in ("quiet_hours_enabled", "daytime_inactivity_enabled"):
            if type(values[key]) is not bool:
                raise ValueError(f"{key} must be boolean")
        for key in ("quiet_start_minute", "quiet_end_minute", "daytime_start_minute", "daytime_end_minute"):
            if type(values[key]) is not int or not 0 <= values[key] <= 1439:
                raise ValueError(f"{key} must be minute 0..1439")
        if type(values["door_open_timeout_seconds"]) is not int or not 30 <= values["door_open_timeout_seconds"] <= 86400:
            raise ValueError("door_open_timeout_seconds must be 30..86400")
        if type(values["daytime_inactivity_seconds"]) is not int or not 300 <= values["daytime_inactivity_seconds"] <= 86400:
            raise ValueError("daytime_inactivity_seconds must be 300..86400")
    ...
    def sync(self):
        ...
        for signal in signals:
            if signal["kind"] not in {"DOOR_LEFT_OPEN", "DAYTIME_INACTIVITY"}:
                continue
            kind = signal["kind"]
            event = {
                "event_id": f"rule:{signal['stable_key']}",
                "kind": kind,
                "location": "entry" if kind == "DOOR_LEFT_OPEN" else "home",
    ...
    def snapshot(self):
        ...
        return {"simulation": {k: v for k, v in self.state.items() if k != "events"},
                ...
                "limitations": ["Synthetic sensors, clock, transport and notification provider",
                                ...
                                "Fault, temperature and battery diagnoses are injected simulations until hardware telemetry exists",
                                "No physical prompts, SMS, push, OTA, RF or real battery simulation"]}
    ...
    def _pwa_rebuild_base(self):
        ...
        battery = dict(getattr(self, "pwa_battery", {}))
        self.reset()
        self.pwa_flags.update(flags)
        self.pwa_audit = audit
        self.pwa_battery.update(battery)
        ...
        if self.pwa_flags["battery_negative"]:
            ...
            self.pwa_battery["kitchen"] = {"percent": 5, "runtime": "~2 hrs"}
            self.sync()
    ...
    def pwa_toggle(self, scenario):
        ...
        if scenario == "morning":
            ...
            if self.pwa_flags["door_negative"]:
                self.action({"action":"event","node":"entry","kind":"DOOR_OPEN"})
                self._pwa_add_audit("Main door opened", "red", "🚪")
            else:
            ...
            if self.pwa_flags["battery_negative"]:
                self._inject_fault("kitchen", "LOW_BATTERY")
                self.pwa_battery["kitchen"] = {"percent": 5, "runtime": "~2 hrs"}
                self._pwa_add_audit("Kitchen sensor battery low: 5%", "red", "🔋")
            else:
                self._clear_fault("kitchen")
                self.pwa_battery["kitchen"] = {"percent": 68, "runtime": "~9 days"}
                self._pwa_add_audit("Kitchen sensor battery restored: 68%", "green", "🔋")
            self.sync()
        return self.pwa_view()
    ...
    def pwa_view(self):
        ...
        care_alert = any(i.get("state") != "RESOLVED" and i.get("kind") in care_kinds for i in snap["incidents"])
        if (self.pwa_flags.get("morning_negative") or self.pwa_flags.get("ok_negative")
                or self.pwa_flags.get("night_negative") or self.pwa_flags.get("door_negative")):
            care_alert = True
        recent = []
        ...
        for d in snap["devices"]:
            pct = self.pwa_battery.get(d["id"], {"percent": None, "runtime":"calculating"})
            ...
            devices.append({
                ...
                "battery": pct["percent"], "left": pct["runtime"], "updated": "just now",
            })
        low = min((d for d in devices if d["battery"] is not None), key=lambda d:d["battery"])
        morning_ok = not self.pwa_flags.get("morning_negative", False)
        ok = not self.pwa_flags.get("ok_negative", False)
        night_visits = 8 if self.pwa_flags.get("night_negative") else 2
        return {
            "source": "Ghar Sajag v1.5.4 C++ rules + Python backend + PWA bridge",
            "simulation_now": now,
            "care": {"alert": care_alert, "title": "Attention needed at home" if care_alert else "Home looks normal", "subtitle": "Please check the home status." if care_alert else "All is well at home."},
            "morning": {"ok": morning_ok},
            "iam_ok": {"ok": ok},
            "door": {"open": door.get("state") == "OPEN" or bool(self.pwa_flags.get("door_negative")), "open_for_s": max(0, now-int(door.get("since") or now)) if door.get("state") == "OPEN" else 0, "indoor_activity_age_min": indoor_age_min},
            "night": {"visits": night_visits, "unusual": night_visits >= 6},
            "device_health": {"low_id": low["id"], "low_name": low["name"], "low_percent": low["battery"], "runtime": low["left"], "attention": low["battery"] <= 10},
            "devices": devices,
            "events": recent,
