...
- A **declarative 68-scenario functional regression catalog** covering expected, unexpected, boundary, failure/recovery, configuration, transport and diagnostic paths.
- A **generated 68-case manual validation plan** sourced from the same catalog, so manual and automated tests do not drift.
...
A host release-gate PASS does **not** certify physical hardware. Real ESP32 GPIO/debounce, ESP-NOW RF/RSSI, battery/UPS, brown-out/reset/watchdog, flash durability, OTA and real notification delivery remain physical acceptance gates.
...
The bridge translates supported controls into the existing v1.5.4 C++/Python simulation path. Device battery percentage/runtime and the bathroom-visit count remain verification telemetry until physical node telemetry is connected.
...
## Parivar Sathi PWA: all 68 canonical scenarios
Run `make pwa-e2e-test` to validate all 68 v1.5.4 scenarios through backend HTTP and the actual PWA DOM. The temporary left-side validation panel also supports manual per-scenario execution and a Run all 68 action.
...
- The PWA's **Run all 68 automatically** flow completes with a visible **68/68 PASS** result.
