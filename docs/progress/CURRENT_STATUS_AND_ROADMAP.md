...
| Hub event journal/dedupe | **PARTIAL** | The authenticated target owner now attaches the existing 128-slot encrypted NVS journal, restores event identities before accepting traffic, and commits/readbacks each new record before its application ACK. The Hub build passed with `GS_HIL_BUILD=OFF`; target restart/power-cut behavior is not physically qualified. Append-only capacity fails closed at 128 with no reclamation, cloud-ACK status remains volatile, and production key protection/endurance remain open. |
